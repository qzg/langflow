...
| `WASM_OCI_BASE` | String | `ghcr.io/ibm/langflow` | Base registry/org[/namespace] for WASM OCI publishes. The UI will append `/{component_slug}:{short_code_hash}` (12-char tag) by default. |
| `WASM_PARITY_STALE_HOURS` | Integer | `24` | Hours after which parity status is considered stale in the UI. |
